...
const ProfileTabs = ({ activeTab, setActiveTab, user, project, run_effect, set_run_effect }) => {
  ...
  // Fetch saved projects when tab is active
  useEffect(() => {
    if (activeTab === 'saved') {
      fetchSavedProjects();
    }
  }, [activeTab, run_effect]);
  ...
  const tabs = [
    ...
    { id: 'saved', label: 'Saved', icon: 'fa-bookmark', count: savedProjects?.length },
...
// Saved Projects Tab
const SavedProjectsTab = ({ savedProjects, loading, backend_url, onUpdate }) => {
  const navigate = useNavigate();
  const { user } = useContext(Context);
  const { unsaveProject } = useProject();

  const handleUnsave = async (projectId) => {
    const result = await unsaveProject(projectId);
    if (result) {
      onUpdate(); // Refresh the saved projects list
    }
  };
      ...
      {savedProjects?.length > 0 ? (
        <div className='saved-projects-grid'>
          {savedProjects.map((project) => (
            <div key={project._id} className='saved-project-card'>
              <div className='saved-project-image'>
                <img
                  src={project.image || `${backend_url}/public/images/default_project.jpg`}
                  alt={project.name}
                  onError={(e) => {
                    e.target.onerror = null;
                    e.target.src = `${backend_url}/public/images/default_project.jpg`;
                  }}
                />
                <button 
              ...
              <div className='saved-project-info'>
                <h3>{project.name}</h3>
                <p className='saved-project-desc'>
                  {project.description?.substring(0, 100)}
                  {project.description?.length > 100 ? '...' : ''}
                </p>
                <div className='saved-project-owner'>
                  <img
                    src={project.owner?.photo || `${backend_url}/public/images/default.jpg`}
        ...
        <EmptyState 
          ...
          description="Projects you save will appear here for easy access"
          buttonText="Explore Projects"
          buttonIcon="fa-compass"
          link="/allproject"
        />
      )}
    </div>
