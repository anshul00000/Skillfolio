...
export default function Project({
  ...
  // Helper functions for like/save status
  ...
  const handleLike = async (projectId, isLiked) => {
    ...
    if (updatedProject) {
      setLocalProjects(prev => prev.map(p => p._id === projectId ? updatedProject : p));
    }
  };
                  ...
                  <div className="card_body">
                    ...
                    <div className="content">
                      ...
                      <div className="project-interactions">
                        <button
                          className={`interaction-btn ${isLiked(p) ? 'liked' : ''}`}
                          onClick={() => handleLike(p._id, isLiked(p))}
                          disabled={!user || loading}
                          title={user ? (isLiked(p) ? 'Unlike' : 'Like') : 'Login to like'}
                        >
                          <i className={`fa-${isLiked(p) ? 'solid' : 'regular'} fa-heart`}></i>
                          <span>{p.likes?.length || 0}</span>
                        </button>

                        <button
                          className={`interaction-btn ${isSaved(p) ? 'saved' : ''}`}
                          onClick={() => handleSave(p._id, isSaved(p))}
                          disabled={!user || loading}
                          title={user ? (isSaved(p) ? 'Unsave' : 'Save') : 'Login to save'}
                        >
                          <i className={`fa-${isSaved(p) ? 'solid' : 'regular'} fa-bookmark`}></i>
                          <span>{p.savedBy?.length || 0}</span>
                        </button>

                        <Comments
                          projectId={p._id}
                          comments={p.comments || []}
                          onUpdate={(updatedProject) => handleCommentUpdate(p._id, updatedProject)}
                        />
                      </div>
                    </div>
                  </div>
