...
function Hero() {
  ...
  useEffect(() => {
    ...
    // Listen for joined_success
    socketRef.current.on("joined_success", (data) => {
      // connected
    });
    ...
    return () => {
      if (socketRef.current) {
        socketRef.current.off("joined_success");
        socketRef.current.disconnect();
      }
    };
  }, [user?._id, user?.email, backend_url]);
